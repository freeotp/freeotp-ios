...
class TokenIcon {
    ...
    func getIcon(issuer: String, imageSize: CGSize) -> UIImage? {
        // Retrieve(if any) existing custom mapping
        if let iconMapping = loadMapping(issuer: issuer) {
            if faIconsSolid.values.contains(iconMapping["Name"]!) {
                if let iconName = FontAwesome(rawValue: iconMapping["Name"]!) {
                    return UIImage.fontAwesomeIcon(name: iconName, style: .solid, textColor: .white, size: imageSize)
                }
            }
            if faIconsBrands.values.contains(iconMapping["Name"]!) {
                if let iconName = FontAwesome(rawValue: iconMapping["Name"]!) {
                    return UIImage.fontAwesomeIcon(name: iconName, style: .brands, textColor: .white, size: imageSize)
                }
            }
        // Fallback to issuer - brand icon lookup
        } else {
            let faIconName = "fa-" + issuer
    ...
    func getColor(issuer: String) -> UIColor? {
        if let iconMapping = loadMapping(issuer: issuer) {
            if let iconColor = iconMapping["Color"] {
                return UIColor(hexString: iconColor)
            }
        } else {
    ...
    func saveMapping(issuer: String, iconName: String, iconColor: String) {
        let iconDict = ["Name": iconName, "Color": iconColor]
        defaults.set(iconDict, forKey: issuer)
    }

    func loadMapping(issuer: String) -> [String: String]? {
    ...
    func removeMapping(issuer: String) {
        defaults.removeObject(forKey: issuer)
